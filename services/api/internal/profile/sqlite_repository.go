...
func (r *SQLiteRepository) Get(ctx context.Context, patientID string) (Profile, error) {
	...
	if err != nil {
		return Profile{}, fmt.Errorf("list allergies: %w", err)
	}
	defer allergyRows.Close()
	result.Allergies = []Allergy{}
	for allergyRows.Next() {
		var item Allergy
		if err := allergyRows.Scan(&item.Name, &item.Severity); err != nil {
			return Profile{}, err
		}
		result.Allergies = append(result.Allergies, item)
	}
	...
	if err != nil {
		return Profile{}, fmt.Errorf("list medications: %w", err)
	}
	defer medicationRows.Close()
	result.Medications = []Medication{}
	for medicationRows.Next() {
		var item Medication
		if err := medicationRows.Scan(&item.Name, &item.Details); err != nil {
			return Profile{}, err
		}
...
func (r *SQLiteRepository) Upsert(ctx context.Context, actorID string, input Profile) (Profile, error) {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO users(id, role, created_at, updated_at) VALUES(?, 'patient', ?, ?) ON CONFLICT(id) DO UPDATE SET updated_at = excluded.updated_at`, input.PatientID, now, now); err != nil {
		return Profile{}, err
	}
	...
	for index, item := range input.Allergies {
		if strings.TrimSpace(item.Name) == "" {
			continue
		}
		if _, err := tx.ExecContext(ctx, `INSERT INTO patient_allergies(id, patient_id, name, severity) VALUES(?, ?, ?, ?)`, input.PatientID+"-allergy-"+strconv.Itoa(index+1), input.PatientID, strings.TrimSpace(item.Name), strings.TrimSpace(item.Severity)); err != nil {
			return Profile{}, err
		}
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM patient_medications WHERE patient_id = ?`, input.PatientID); err != nil {
		return Profile{}, err
	}
	for index, item := range input.Medications {
		if strings.TrimSpace(item.Name) == "" {
			continue
		}
		if _, err := tx.ExecContext(ctx, `INSERT INTO patient_medications(id, patient_id, name, details) VALUES(?, ?, ?, ?)`, input.PatientID+"-medication-"+strconv.Itoa(index+1), input.PatientID, strings.TrimSpace(item.Name), strings.TrimSpace(item.Details)); err != nil {
			return Profile{}, err
		}
	}
	if _, err := tx.ExecContext(ctx, `INSERT INTO audit_events(id, actor_id, subject_id, action, resource_type, resource_id, occurred_at) VALUES(?, ?, ?, 'profile.upsert', 'patient_profile', ?, ?)`, input.PatientID+"-audit-"+strconv.FormatInt(time.Now().UnixNano(), 10), actorID, input.PatientID, input.PatientID, now); err != nil {
		return Profile{}, err
	}
