...
func (r *SQLiteRepository) List(ctx context.Context, patientID string) ([]Link, error) {
	...
	for rows.Next() {
		var item Link
		if err := rows.Scan(&item.ID, &item.OtherPatientID, &item.OtherDisplayName, &item.Relationship, &item.Status, &item.Direction, &item.ShareFamilyHistory, &item.CreatedAt); err != nil {
			return nil, err
		}
...
func (r *SQLiteRepository) Invite(ctx context.Context, patientID, email, relationship string) (Link, error) {
	...
	if err := r.db.QueryRowContext(ctx, `SELECT u.id, a.display_name FROM auth_accounts a JOIN users u ON u.id=a.user_id WHERE a.email=? AND u.role='patient'`, email).Scan(&relativeID, &displayName); err != nil || relativeID == patientID {
	...
	if err := r.db.QueryRowContext(ctx, `SELECT id, created_at FROM family_links WHERE requester_patient_id=? AND relative_patient_id=?`, patientID, relativeID).Scan(&id, &now); err != nil {
		return Link{}, err
	}
...
func (r *SQLiteRepository) Respond(ctx context.Context, patientID, linkID string, accept, share bool) (Link, error) {
	...
	links, err := r.List(ctx, patientID)
	if err != nil {
		return Link{}, err
	}
