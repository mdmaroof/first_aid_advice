...
func Migrate(ctx context.Context, db *sql.DB) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS appointments (
			...
			status TEXT NOT NULL CHECK (status IN ('scheduled', 'checked_in', 'completed', 'cancelled')),
			created_at TEXT NOT NULL
		)`,
		`CREATE INDEX IF NOT EXISTS idx_appointments_clinic_time ON appointments(clinic_id, scheduled_at)`,
		...
		`CREATE INDEX IF NOT EXISTS idx_encounters_patient_time ON encounters(patient_id, created_at DESC)`,
		...
		`CREATE INDEX IF NOT EXISTS idx_prescriptions_patient_time ON prescriptions(patient_id, created_at DESC)`,
		...
		`CREATE INDEX IF NOT EXISTS idx_labs_patient_time ON lab_orders(patient_id, ordered_at DESC)`,
