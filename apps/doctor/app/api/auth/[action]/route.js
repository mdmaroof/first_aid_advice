...
export async function POST(request, { params }) {
  ...
  try {
    const body = action === "signout" ? undefined : { ...(await request.json()), role: "doctor" };
    ...
    if (action === "signout") { cookies().delete(sessionCookie); return new NextResponse(null, { status: 204 }); }
