...
export function SharedProfile({ patientID }) {
  ...
  useEffect(() => { fetch(`/api/curais/patients/${encodeURIComponent(patientID)}`, { cache: "no-store" }).then(readApiResponse).then(setProfile).catch((reason) => setError(errorMessage(reason, "Unable to load profile."))); }, [patientID]);
