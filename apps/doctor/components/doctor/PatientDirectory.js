  ...
  return (
    <div className="mt-8">
      <form onSubmit={(event) => { event.preventDefault(); load(query); }} className="glass flex max-w-2xl items-center gap-3 rounded-2xl p-2 pl-4">
        ...
        <input id="patient-query" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search shared patients by name or exact mobile" className="min-w-0 flex-1 bg-transparent py-2 text-sm text-aid-ink outline-none placeholder:text-aid-muted" />
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {patients.map((patient) => <a href={`/patients/${encodeURIComponent(patient.patientId)}`} key={patient.patientId} className="glass-strong rounded-[1.5rem] p-5 transition hover:-translate-y-1"><span className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-aid-teal/10 text-aid-teal"><UserRound className="h-5 w-5" /></span><h2 className="mt-4 font-quicksand text-xl font-bold">{patient.displayName}</h2><p className="mt-2 text-sm text-aid-muted">Blood group: {patient.bloodGroup || "Not recorded"}</p><p className="mt-1 text-sm text-aid-muted">Mobile: {patient.mobilePhone || "Not recorded"}</p><p className="mt-3 text-xs font-bold uppercase tracking-[0.12em] text-aid-teal">Active profile grant</p></a>)}
      </div>
